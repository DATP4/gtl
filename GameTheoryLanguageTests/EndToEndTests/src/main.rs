...
let mut gamestate: GameState = GameState {turn: 1,players: Vec::new(),moves_and_points: Vec::new(), 
};let x = 5
;
assert_eq!(x, 5)
}
...
assert_eq!(p.game_state.player_score("p3".to_string()), 7);
}
}